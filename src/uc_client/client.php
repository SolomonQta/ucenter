...
/*
 * $returnsrc 参数 当为TRUE时函数返回头像路径，为FALSE时返回带img标签的路径
 * $size 为头像有大小
 *
 * */
function discuz_uc_avatar($uid, $size = 'middle', $returnsrc = TRUE, $type = 'virtual') {
	...
	if($res == 1 && $uid > 0) {
		...
		return $returnsrc ? UC_API.'/images/noavatar_'.$size.'.gif' : '<img src="'.UC_API.'/images/noavatar_'.$size.'.gif" />';
	}
...
* @uid : ucenter 用户id
* */
function uc_avatar_url($uid, $type= 'virtual'){
	$uid = intval($uid);
    $uc_input = uc_api_input("uid=$uid");
    $input = str_replace('25', '', $uc_input);
	$uc_avatarUrl = UC_API.'/index.php?m=user&inajax=1&a=rectavatar&appid='.UC_APPID.'&input='.$input.'&agent='.md5($_SERVER['HTTP_USER_AGENT']).'&avatartype='.$type;
    return $uc_avatarUrl;
}
